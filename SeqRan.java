//this code implements a method that returns a random sequence from an arbitrary alphabet
//the method does the following:
//checks to see that alphabet and weights have the same length
//checks to see that length >= 0
//checks to see that the sum of weights is within round-off error of 1
//Returns a random string of characters sampled with replacement from alphabet[] with relative frequencies set by weights[]
	...
	public static String generateRandomSequence(char[] alphabet, float[] weights, int length) throws Exception 
	
		{
			...
			//checks that the round off error is correct
                        if(Math.abs(sum-1) > 0.01) throw new Exception("sum of weights is not within round off error of 1");
					...
					
					{
						...
						{			
							...
							sumWeights = sumWeights + weights[count];
							//increase count by 1
							count = count + 1;
